...
loc_colors <- c("#D81B60","#004D40","#1E88E5","purple4","orange") %>% set_names(loc_labels)
...
outliers_list <- 
  DescTools::Outlier(ds_sum$unrestrained_prop, method = "boxplot")
ds_sum_excl2 <- 
  ds_sum[!(ds_sum$unrestrained_prop %in% outliers_list),]
...
# save(ds, ds_sum, synced_ppts, id_info, id_demo, ds_human, ds_sum_excl, ds_sum_excl2, file = "whole-model-prediction.RData")
```
...
ggsave("figure/age_related_change.png",width=6, height=4)
ggsave("figure/age_related_change.eps",width=6, height=4)
summary(lm(unrestrained_prop ~ agemo, data = ds_sum_excl2))

ggplot(ds_sum_excl2) +
  geom_point(aes(x = agemo, y = unrestrained_prop),size=4, shape=1, stroke=1.5) +
...
summary(lm(unrestrained_prop ~ scale(agemo), data = ds_sum_excl2%>% filter(age_group=="Younger")))
```
...
summary(lm(unrestrained_prop ~ scale(agemo), data = ds_sum_excl2%>% filter(age_group=="Older")))
```
...
## Q3 locomotor-related change: crawling + sitting( + walking)
```{r}
ds_sum %>% 
...
mean_unr_loc <- ds_sum_excl2 %>% 
  group_by(loc) %>% 
  summarize(age = mean(agemo),
            prop = mean(unrestrained_prop),
            .groups="drop") %>% 
  mutate(x=age-0.8,
         xend=age+0.8,
         y=prop,
...
ds_sum_excl2 %>% 
  ggplot() +
    geom_point(aes(x = agemo, y = unrestrained_prop, color = loc),size=4,shape=1, stroke=1.5) + 
    scale_x_continuous(name = "Age (mon)", breaks = c(4,7,11,14), limits = c(3,15)) +
    scale_y_continuous(name = "Unrestrained time (%)", breaks = seq(0, 1, .25), limits = c(0,1))+
    scale_color_manual(values=loc_colors)
ggsave("figure/sit_crawl_walk_change.png", width=8, height=4)
ggsave("figure/sit_crawl_walk_change.eps", width=8, height=4)
...
summary(lm(unrestrained_prop ~ agemo, data = ds_sum_excl2))
summary(lm(unrestrained_prop ~ sitter + agemo, data = ds_sum_excl2))
summary(lm(unrestrained_prop ~ sitter + crawler + agemo, data = ds_sum_excl2))
summary(lm(unrestrained_prop ~ sitter + crawler + walker + agemo, data = ds_sum_excl2))
```
