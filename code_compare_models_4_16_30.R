...
ds_16_no_pos <- read_csv("metrics_restrained_16_no_pos.csv") %>% 
  mutate(id = ifelse(str_detect(id, "-"), format(as.Date(id, format="%d-%b"),"%m/%d"),id)) %>% 
  arrange(id)%>% 
  rename(id=id, acc=overall_acc, sens=overall_sens, spec=overall_spec, posi=overall_posi_pred) %>% 
  mutate(sens = 1-sens, spec = 1-spec, posi = 1-posi)
...
# merge data frames
ds_merge <- reduce(list(ds_30_no_pos,ds_16_no_pos, ds_4_no_pos), full_join, by = "id")
colnames(ds_merge) <- c("id","acc_30", "sens_30", "spec_30", "posi_30",
                        "acc_16", "sens_16", "spec_16", "posi_16",
                        "acc_4", "sens_4", "spec_4", "posi_4")

ds_long <- bind_rows(list(ds30 = ds_30_no_pos, ds16 = ds_16_no_pos, ds4 = ds_4_no_pos),
                     .id = "model") %>% 
  mutate(model = factor(model, levels=c("ds30","ds16","ds4"),
                        labels = c("ds30","ds16","ds4")))
...
# statistical test
# accuracy
t.test(ds_merge$acc_30, ds_merge$acc_16, paired = TRUE, alternative = "two.sided", na.action = na.omit)
t.test(ds_merge$acc_30, ds_merge$acc_4, paired = TRUE, alternative = "two.sided", na.action = na.omit)
t.test(ds_merge$acc_16, ds_merge$acc_4, paired = TRUE, alternative = "two.sided", na.action = na.omit)
mod1 <- aov(acc ~ model, data = ds_long)
summary(mod1)
shapiro.test(residuals(mod1)) # Normality of Residuals NOT met
leveneTest(mod1) # Homogeneity of Variance met

# sensitivity
t.test(ds_merge$sens_30, ds_merge$sens_16, paired = TRUE, alternative = "two.sided", na.action = na.omit)
t.test(ds_merge$sens_30, ds_merge$sens_4, paired = TRUE, alternative = "two.sided", na.action = na.omit)
t.test(ds_merge$sens_16, ds_merge$sens_4, paired = TRUE, alternative = "two.sided", na.action = na.omit)
summary(aov(sens ~ model, data = ds_long))

# specificity
t.test(ds_merge$spec_30, ds_merge$spec_16, paired = TRUE, alternative = "two.sided", na.action = na.omit)
t.test(ds_merge$spec_30, ds_merge$spec_4, paired = TRUE, alternative = "two.sided", na.action = na.omit)
t.test(ds_merge$spec_16, ds_merge$spec_4, paired = TRUE, alternative = "two.sided", na.action = na.omit)
summary(aov(spec ~ model, data = ds_long))

# positive predictive value
t.test(ds_merge$posi_30, ds_merge$posi_16, paired = TRUE, alternative = "two.sided", na.action = na.omit)
t.test(ds_merge$posi_30, ds_merge$posi_4, paired = TRUE, alternative = "two.sided", na.action = na.omit)
t.test(ds_merge$posi_16, ds_merge$posi_4, paired = TRUE, alternative = "two.sided", na.action = na.omit)
summary(aov(posi ~ model, data = ds_long))
...
#   facet_wrap(~ facet)
# ggsave("figure/acc_across_models_v2.eps", width = 15, height = 20)
...
#           theme(legend.position = "none")
#   ggsave(paste("figure/",ind,"_v2.eps",sep=""), width = 5, height = 10)
# }
...
ggsave("figure/acc_variability_v2.png", width = 30, height = 10)
...
ggsave(paste("figure/barplot_v2.png",sep=""), width = 10, height = 5)
