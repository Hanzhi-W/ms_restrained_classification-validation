...
#| apa-note: Age in months served as a continuous predictor and was centered within each age group; skill was a categorical predictor with non-sitters/non-walkers serving as the reference categories
#| tbl-cap: POSTPONE-Models' prediction performance for unrestrained periods compared to restrained periods
#| ft.align: left
...
#
t <- matrix_comb %>% 
  filter (model=="30_no_pos") %>% 
  select(id, session, positive_class, sensitivity) %>% 
  pivot_wider(names_from = positive_class, values_from = sensitivity) %>% 
  mutate(df = u-r)
summary(lm(df ~ 1, data = t))
...
tbl_ctgr_16 <- lm_ctgr("16_no_pos") %>% 
  ...
  mutate(p = ifelse(p < .001,"<.001",round(p,3)),
         p = str_remove(p, "^0")) %>% 
  mutate(across(where(is.numeric), ~ round(.x, 3))) %>% 
  add_column(Term = rep(c("Int.","Unrest. vs Rest."), 4), .before = "Estimate") %>% 
  select(-SE)
...
tbl_ctgr <- cbind(tbl_ctgr_4, tbl_ctgr_16 %>% select(-Indices,-Term) %>% rename(Estimate_16=Estimate,t_16=t,p_16=p)) %>% 
  cbind(tbl_ctgr_30 %>% select(-Indices, -Term) %>% rename(Estimate_30=Estimate,t_30=t,p_30=p))

tbl_flex_ctgr <- tbl_ctgr %>% 
  flextable() %>% flextable::theme_apa() %>% line_spacing(part = "all") %>%
  set_header_labels(Estimate_16 = "Estimate", t_16 = "t", p_16 = "p",
                    Estimate_30 = "Estimate", t_30 = "t", p_30 = "p") %>%
  merge_v(j = "Indices") %>% fix_border_issues() %>% 
  hline(i = c(2,4,6,8)) %>% 
  padding(padding.top = 2, padding.bottom = 2) %>% 
  set_table_properties(layout = "autofit", width = .99) %>% 
  align(align = "center", part = "all") %>% 
  add_header_row(values = c("", "4s Model", "16s Model", "30s Model"), colwidths = c(2,3,3,3))
tbl_flex_ctgr
```
