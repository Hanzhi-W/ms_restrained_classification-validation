...
p_ci_sen <- ggplot (table_btw_ctg %>% 
  ...
  scale_y_continuous(
    limits = c(-0.15, 0.1),
    breaks = c(-0.15,-0.1, -0.05, 0, 0.05, 0.1, 0.1)
  )+
  theme_update() +
...
p_ci_ppv <-ggplot (table_btw_ctg %>% 
  ...
  annotate("text", x = 2.6, y = 0.085, label="The model's prediction \n of restrained bouts \n is more credible",color = "black")

(p_ci_sen + p_ci_ppv)
```

```{r tbl-prevalencecor}
#| apa-note: Prevalence was calculated as the proportion of restrained periods among all window samples for each session. Prediction error was calculated as model predicted prevalence minus human annotated prevalence. A negative prediction error means model prediction underestimates the restrained time compared to human annotation. M = mean difference; CI = bootstrapped confidence interval. 
...
table_flex_prev <- table_prev_100 %>% 
  flextable() %>% flextable::theme_apa() %>% line_spacing(part = "all") %>% 
  fix_border_issues() %>% 
  add_header_row(values = c("","Mean of Prevalence (%)", "","Prediction Error (%)"), colwidths = c(1,2,1,2)) %>% 
  hline(i = c(1,2,3)) %>% 
  padding(padding.top = 2, padding.bottom = 2) %>% 
  set_table_properties(layout = "autofit", width = .99) %>% 
  align(align = "center", part = "all")
...
#| fig-width: 11
#| fig-height: 5
#| apa-note: Prevalence is defined as the proportion of windows predicted (or annotated) as restrained among all windows. (A) shows the correlation of prevalence between model prediction and human annotation. The black dashed line denotes the reference of y=x. Each dot is a session for a specific window length. (B) shows the difference of prevalence between human annotation and model prediction (i.e., prediction error). A negative prediction error indicates the model estimated less amount of time infants were restrained compared to human annotation. The line in the box denotes the median; the boarder lines of the box denotes the first (Q1) quartile and the third (Q3) quartile; and the lines extended denotes the min and max data points within the range of 1.5 IQR (i.e., Q3-Q1).
...
p_prev_cor <- ggplot(data=matrix_comb %>% filter(positive_class=="r") %>%
  ...
  geom_text(data = table_prev,
            aes(x = c(1.15,1.15,1.15),
                y = c(.7,.8,.9),
                label = paste0("r = ", round(cor,2)),
                color = model), 
            show.legend = FALSE) +
  scale_color_manual(values=model_colors, labels=c("4 s", "16 s","30 s")) +
  scale_x_continuous(limits = c(0,1.3), breaks=c(0,.5,1)) +
...
p_prev_gap <- ggplot(data=matrix_comb %>% filter(positive_class=="r") %>%
  ...
  labs(x="Window Length", y="Prediction Error (%)")+
  guides(color = "none")+
  theme_update()+
...
#| fig-width: 4
#| fig-height: 3
#| apa-note: Each point denotes one session. The line denotes the group mean of restrained time (%)
