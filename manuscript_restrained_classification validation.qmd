...
#| apa-note: 
#| tbl-cap: Performance metrics for each window length and ANOVA comparison across window lengths
#| ft.align: left
...
```{r tbl-cibtwctg}
#| apa-note: Differences were calculated as restrained – unrestrained metric value. M = mean difference; CI = bootstrapped confidence interval. All window lengths had lower sensitivity and higher PPV for restrained period detection.
#| tbl-cap: Differences in sensitivity and positive predictive value (PPV) between restrained and unrestrained prediction across window lengths
#| ft.align: left
...
```{r fig-cibtwctg}
#| fig-cap: Differences in sensitivity and (PPV) between restrained and unrestrained prediction across window lengths
#| fig-width: 8
#| fig-height: 5
#| apa-note: Each vertical line indicates the 95%CI of the metric's difference between restrained prediction and unrestrained prediction. The dot on each line denotes the mean of the difference. All window lengths had higher sensitivity for unrestrained period prediction and higher ppv for restrained period prediction.
...
table_flex_prev <- table_prev_100 %>% 
  flextable() %>% flextable::theme_apa() %>% line_spacing(part = "all") %>% 
  fix_border_issues() %>% 
  add_header_row(values = c("","Prevalence (%)", "Prediction Error (%)"), colwidths = c(1,3,3)) %>% 
  hline(i = c(1,2,3)) %>% 
  padding(padding.top = 2, padding.bottom = 2) %>% 
  set_table_properties(layout = "autofit", width = .99) %>% 
  align(align = "center", part = "all")
...
```{r fig-prevalencegap}
#| fig-cap: Prediction error between model prediction and human annotation
#| fig-width: 6
#| fig-height: 5
#| apa-note: Prediction error is the difference in the overall restrained time between the model estimate and the human annotation. A negative prediction error indicates the model underestimate restrained time overall compared to human annotation. Each point denotes one session. The violin shape denotes the distribution of prediction error of each window length. The box denotes the first (Q1) quartile, the median, and the third (Q3) quartile of prediction error.
